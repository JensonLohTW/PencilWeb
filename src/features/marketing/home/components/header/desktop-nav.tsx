    ...
    return (
        <PopoverGroup className="hidden lg:flex lg:gap-x-12">
            {headerNavigation.map((item) =>
                item.type === 'dropdown' ? (
                    <Popover key={item.nameKey} className="relative">
                        <PopoverButton className="flex items-center gap-x-1 text-sm/6 font-medium text-white outline-none">
                            {t(item.nameKey)}
                        ...
                        <PopoverPanel
                            transition
                            className="absolute top-full -left-8 z-10 mt-3 w-screen max-w-md overflow-hidden rounded-3xl bg-gray-800 outline outline-1 -outline-offset-1 outline-white/10 ring-1 ring-gray-900/5 transition data-[closed]:translate-y-1 data-[closed]:opacity-0 data-[enter]:duration-200 data-[leave]:duration-150 data-[enter]:ease-out data-[leave]:ease-in"
                        >
                            <div className="p-4">
                                {item.children?.map((child) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-auto">
                                            <Link href={child.href || '#'} className="block font-semibold text-white">
                                                {t(child.nameKey)}
