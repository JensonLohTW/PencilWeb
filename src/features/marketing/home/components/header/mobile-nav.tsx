...
export function MobileNav({ mobileMenuOpen, setMobileMenuOpen, utilities, actions }: MobileNavProps) {
  ...
  return (
    <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className="lg:hidden">
      ...
      <DialogPanel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-black/70 p-6 backdrop-blur-2xl backdrop-saturate-150 sm:max-w-sm sm:border-l sm:border-white/10 sm:shadow-[-24px_0_48px_-12px_rgba(0,0,0,0.5)]">
                    ...
                    <DisclosurePanel className="mt-2 space-y-2">
                      {item.children?.map((child) => (
                        <DisclosureButton
                          ...
                          as={Link}
                          href={child.href || '#'}
                          className="block rounded-lg py-2 pr-3 pl-6 text-base/7 font-bold text-white hover:bg-white/5"
                          onClick={() => setMobileMenuOpen(false)}
                        >
                          {t(child.nameKey)}
                        </DisclosureButton>
