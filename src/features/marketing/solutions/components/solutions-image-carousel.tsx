...
export function SolutionsImageCarousel() {
  ...
  return (
    <div className="group relative mx-auto w-full max-w-[76rem] overflow-hidden rounded-xl bg-pencil-900/5 p-2 ring-1 ring-pencil-900/10 ring-inset lg:rounded-2xl lg:p-4 dark:bg-white/5 dark:ring-white/10">
      <div className="relative aspect-[16/10] w-full overflow-hidden rounded-md bg-white shadow-xl ring-1 ring-pencil-900/10 sm:aspect-[16/9] dark:bg-pencil-900 dark:ring-white/10">
        <AnimatePresence initial={false} custom={direction} mode="popLayout">
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-6 sm:p-8">
              <h3 className="text-xl font-semibold text-white sm:text-2xl">{slides[currentIndex].title}</h3>
            </div>
          </motion.div>
        </AnimatePresence>
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${
              index === currentIndex ? 'w-8 bg-cta' : 'w-2 bg-white/50 hover:bg-white/80'
            }`}
