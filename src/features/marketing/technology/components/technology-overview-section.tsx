    ...
    <section className="py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <FadeIn className="mx-auto max-w-2xl lg:text-center">
          <h2 className="text-base/7 font-semibold text-accent-600 dark:text-accent-400">{overview.eyebrow}</h2>
          ...
          <p className="mt-6 text-lg/8 text-pencil-600 dark:text-pencil-400">{overview.description}</p>
        </FadeIn>

        <div className="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-none">
          <StaggerContainer
            viewport={{ once: true, amount: 0.2 }}
            className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-8 lg:max-w-none lg:grid-cols-4"
          >
            {overview.items.map((item) => (
              <StaggerItem key={item.label}>
                <TiltArticle
                  ...
                  intensity={5}
                >
                  <p className="text-sm font-semibold text-accent-600 dark:text-accent-400">{item.label}</p>
                  <p className="mt-4 text-4xl font-semibold tracking-tight text-pencil-950 dark:text-white">
                    {item.value}
                  </p>
