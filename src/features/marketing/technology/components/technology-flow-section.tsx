    ...
    <section id="tech-flow" className="py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <FadeIn className="mx-auto max-w-2xl lg:text-center">
          <h2 className="text-base/7 font-semibold text-accent-600 dark:text-accent-400">{flow.eyebrow}</h2>
          ...
          <p className="mt-6 text-lg/8 text-pencil-600 dark:text-pencil-400">{flow.description}</p>
        </FadeIn>
        ...
        >
          {flow.stats.map((stat) => (
            ...
            >
              <p className="text-sm font-semibold text-pencil-500 dark:text-pencil-400">{stat.label}</p>
              <p className="mt-2 text-3xl font-semibold tracking-tight text-pencil-950 dark:text-white">{stat.value}</p>
            </StaggerItem>
          ))}
        </StaggerContainer>
        ...
        <div className="mx-auto mt-16 max-w-3xl">
          <StaggerContainer viewport={{ once: true, amount: 0.2 }} className="flex flex-col gap-4">
            {flow.steps.map((step, index) => (
              <StaggerItem
                key={step.number}
                className={cn(
                  'group flex cursor-pointer items-start gap-6 rounded-2xl border p-6 transition-all duration-300',
                  activeIndex === index
                  ...
                  <h3
                    className={cn(
                      ...
                      activeIndex === index
                        ...
                        : 'text-pencil-700 group-hover:text-pencil-950 dark:text-pencil-300 dark:group-hover:text-white',
                    )}
                  >
                    {step.title}
                  </h3>
                  <p className="mt-1 text-sm/6 text-pencil-600 dark:text-pencil-400">{step.description}</p>
                </div>
              </StaggerItem>
            ))}
          </StaggerContainer>
